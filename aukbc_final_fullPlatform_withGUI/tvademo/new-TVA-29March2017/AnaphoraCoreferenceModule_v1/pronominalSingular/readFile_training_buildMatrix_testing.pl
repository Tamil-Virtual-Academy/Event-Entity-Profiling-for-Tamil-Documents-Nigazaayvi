...
for($i = 0;$i< @fileContent;$i++) {
		...
		if(($temp[3] =~ /PR\_/)  && ($fileContent[$i][$j] =~ /TRUE$/)) {
			...
			#	if($temp[8] !~ /$tag/) {
					...
					print "Pronoun: $fileContent[$i][$j] - $inpFile - $pn_sentNo - $pn_wrdNo\n";
			...
			#	}
				...
				$outputCRF = `crf_test -m pronominalSingular/newModel_pn_sg testFile.txt`;
				...
				for($ijk123 = 0;$ijk123 < @outArra;$ijk123++) {
					@temppArr123 = split(/\s+/,$arrayOne[$ijk123]);

					if(($pn_sentNo == $temppArr123[0]) && ($temppArr123[1] > $pn_wrdNo)) {
						next;
					}
				...
				if($antecedentFlag == 0) {
					undef %salienceScoreHash;

					
					print STDERR "Need to post-process further\n";
					for($ijk123 = 0;$ijk123 < @outArra;$ijk123++) {
						$score1234 = scoreLine($outArra[$ijk123]);
						print STDERR "$arrayOne[$ijk123] --- $score1234\n";
						$salienceScoreHash{$arrayOne[$ijk123]} = $score1234;
					}

					foreach $k9 (sort{$salienceScoreHash{$b}<=>$salienceScoreHash{$a}} keys%salienceScoreHash) {
						print "$k9\n";
						last;
					}
				}
		...
		if($nptext ne undef) {
			...
			if($fileContent[$new_sentId1][$j1+1] !~ /I\-NP/) {
				...
				if($temp1[3] =~ /PR_/){
					...
					if(($new_g1 =~ /$new_g2/) && ($new_n1 eq $new_n2)) {$pngMatch = 1;}
				}
...
sub scoreLine {
	my $checkLine12345 = $_[0];

	print STDERR "~~~$checkLine12345\n";
	@temp12345 = split(/\s+/, $checkLine12345);

	$tempScore12345 = 0;
	
	if($temp12345[2] =~ /^NPOSS$/i){
#sentence recency
		if($temp12345[5] =~ /yes/i) {
			$tempScore12345 = $tempScore12345 + 100;
		}elsif($temp12345[6] =~ /yes/i) {
			$tempScore12345 = $tempScore12345 + 90;
		}elsif($temp12345[7] =~ /yes/i) {
			$tempScore12345 = $tempScore12345 + 80;
		}elsif($temp12345[8] =~ /yes/i) {
			$tempScore12345 = $tempScore12345 + 70;
		}elsif($temp12345[9] =~ /yes/i) {
			$tempScore12345 = $tempScore12345 + 60;
		}

		if($temp12345[10] =~ /nom/i) {
			$tempScore12345 = $tempScore12345 + 80;
		}elsif($temp12345[11] =~ /acc/i) {
			$tempScore12345 = $tempScore12345 + 50;
		}elsif($temp12345[12] =~ /dat/i) {
			$tempScore12345 = $tempScore12345 + 40;
		}elsif($temp12345[13] =~ /other/i) {
			$tempScore12345 = $tempScore12345 + 30;
		}
		
		if($temp12345[5] =~ /yes/i) {
			if($temp12345[15] =~ /^IC/i) {
				$tempScore12345 = $tempScore12345 + 100;
			}elsif($temp12345[14] =~ /^same/i) {
				$tempScore12345 = $tempScore12345 + 50;
			}elsif($temp12345[16] =~ /^non\_IC/i) {
				$tempScore12345 = $tempScore12345 + 50;
			}
		}
		if($temp12345[6] =~ /yes/i) {
			if($temp12345[17] =~ /mcl/i) {
				$tempScore12345 = $tempScore12345 + 10;
			}
		}
	}elsif($temp12345[1] =~ /^POSS/i){
                if($temp12345[5] =~ /yes/i) {
                        $tempScore12345 = $tempScore12345 + 100;
                }elsif($temp12345[6] =~ /yes/i) {
                        $tempScore12345 = $tempScore12345 + 90;
	...
	print STDERR "score - $tempScore12345\n";
	return($tempScore12345);
}
1;
