...
while(<RE>) {
	chomp;
	next if($_ eq undef);
	if(($_ =~ /(SYM|PUNC)\tI\-NP/) || ($_ =~ /(PUNC|SYM)\tI\-VG/)) {
		next;
	}
	push(@cl_taggedA,$_);
}
...
if(($orginalA[$j] =~ /(SYM|PUNC)\t/) && ($orginalA[$j] !~ /B\-NP/)) {
	print "$orginalA[$j]\to\n";
	$j++;
}
...
# if start tag handle in the chunk starting and if there is end tag then handle in the chunk ending
	...
	$i++;
	if(($orginalA[$j] =~ /(SYM|PUNC)\t/)  && ($orginalA[$j] !~ /B\-(.*?)\t/) ){
		if($cl_taggedA[$i] =~ /\t(\{\/?\w+\})$/) {
			$marker = $1;
			print "$orginalA[$j]","\t$marker\n";
		}else{
			print "$orginalA[$j]","\to\n";
		}
		$j++;
